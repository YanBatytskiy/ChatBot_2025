...
// getters
const std::size_t &Message::getMessageId() const {
  return _messageId;
}

std::weak_ptr<User> Message::getSender() const {
  return _sender;
}

const int64_t &Message::getTimeStamp() const {
  return _time_stamp;
}

const std::vector<std::shared_ptr<IMessageContent>> &Message::getContent() const {
  return _content;
}

// setters
void Message::setMessageId(std::size_t messageId) {
...
void Message::printMessage(const std::shared_ptr<User> &currentUser) {
  ...
  // Determined the sender
  auto sender_ptr = _sender.lock();

  // Determined the message direction
  ...
  if (!messageDirection) { // income Message
    ...
    if (sender_ptr != nullptr) {
      std::cout << "     <- Incoming from Login/Name:    " << sender_ptr->getLogin() << "/" << sender_ptr->getUserName()
                << "    " << formatTimeStampToString(_time_stamp, true) << ", messageId: " << _messageId << std::endl;
    } else {
      std::cout << "     <- Incoming from Login/Name:    " << "User was removed.    "
                << formatTimeStampToString(_time_stamp, true) << ", messageId: " << _messageId << std::endl;
    }
  } else {

    std::cout << "\033[37m"; // white
    std::cout << "-> Outgoing from you: " << currentUser->getUserName() << "    "
              << formatTimeStampToString(_time_stamp, true) << " messageId: " << _messageId << std::endl;
  }
