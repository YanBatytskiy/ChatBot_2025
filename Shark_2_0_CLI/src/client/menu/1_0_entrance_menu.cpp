...
short entranceMenu() { // show the main menu
  while (true) {
    printSystemName();
    ...
    std::cout << "ChatBot 'Shark' Version 2.0. @2025 \n \n"
              << std::endl;

    std::cout << "Select a menu item: " << ::std::endl
              << "1. Register a user " << std::endl
              << "2. Log in to ChatBot" << std::endl
...
bool userLoginInsystem(ClientSession &clientSession) {
  ...
  while (true) {
    try {
      userLogin = inputDataValidation("Enter your login or 0 to exit:", 0, 0,
                                      false, false);
      if (userLogin == "0")
        return false;
  ...
  while (true) {
    try {
      userPassword = inputDataValidation(
        "Enter your password (or 0 to exit):", 0, 0, true, false);
      if (userPassword == "0")
        return false;
...
//
//
void loginMenuChoice(ClientSession &clientSession) { // show the main menu
  int userChoiceNumber;
  std::string userChoice;

  while (true) {
    std::cout << std::endl;
    std::cout << "Good day, user "
              << clientSession.getActiveUserCl()->getUserName() << std::endl;
    std::cout << std::endl;
    std::cout << "Select a menu item: " << std::endl;
    std::cout << "1 - Create a new chat" << std::endl;
    std::cout << "2 - Show chat list" << std::endl;
    std::cout << "3 - Show user profile" << std::endl;
