...
bool clearBaseSQL(PGconn *conn) {
  ...
  std::string sql =
    R"(DROP SCHEMA IF EXISTS public CASCADE;
	  )";
...
//
//
bool checkBaseTablesSQL(PGconn *conn, [[maybe_unused]] std::multimap<int, std::string> sqlMultimap,
                        [[maybe_unused]] std::multimap<int, std::string> sqlDescription) {
  ...
  std::ifstream file_schema(config_dir + "/schema_db.conf");
  if (!file_schema.is_open()) {
    std::cerr << "Missing schema_db.conf in " << config_dir << '\n';
    return false;
  }
  json config;
  try {
    ...
    throw exc::SQLReadConfigException(" schema_db.conf: : JSON error: ");
  }
  ...
  try {
    for (int i = 1; i <= quantity; ++i) {
      ...
      try {
        ...
        throw exc::SQLCreateTableException("schema_db.conf: missing or wrong type for key: " + tableKey);
      }
...
PGresult *execTransactionToSQL(PGconn *conn, std::multimap<int, std::string> &sqlRequests,
  ...
  try {
    ...
    if (!result || PQresultStatus(result) != PGRES_COMMAND_OK) {
      if (result)
        PQclear(result);
      throw exc::SQLExecException("Opening the transaction.");
    }
    PQclear(result);
    int i = 1;
    ...
    std::cerr << ex.what() << "\n"
              << " Check the SQL queries: " << errorTable << "\n";
    ...
    if (PQresultStatus(result) == PGRES_TUPLES_OK && quantity > 0) {
      ...
      for (int i = 0; i < quantity; ++i) {
        ...
        if (PQgetisnull(result, i, 1) == 0) {
          ...
          participantsDTO.lastReadMessage = 0; // or any default value
        }
...
std::optional<std::vector<UserDTO>> getSeveralUsersDTOFromSrvSQL(PGconn *conn, const std::vector<std::string> &logins) {
  ...
  try {
    ...
    // Add all logins to the query
    bool first = true;
    for (const auto &login : logins) {

      if (!first)
        sql += ",\n"; // comma only between elements
    ...
    sql += ")\n";
    sql += R"(
	select pu.login, name, email, phone 
