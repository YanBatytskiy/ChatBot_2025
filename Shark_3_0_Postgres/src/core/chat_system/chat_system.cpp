...
const bool &ChatSystem::getIsServerStatus() const {
  return _isServerStatus;
}

std::size_t &ChatSystem::getNextChatId() {
  return _nextChatIdClient;
}
const std::size_t &ChatSystem::getNextChatId() const {
  return _nextChatIdClient;
}

std::set<std::size_t> &ChatSystem::getFreeChatServerIdSet() {
  return _freeChatServerIdSet;
}
const std::set<std::size_t> &ChatSystem::getFreeChatServerIdSet() const {
  return _freeChatServerIdSet;
}
...
std::deque<std::pair<PacketDTO, std::string>> &ChatSystem::getPacketReceivedDeque() {
  return _packetReceivedDeque;
}

std::deque<std::pair<PacketListDTO, std::string>> &ChatSystem::getPacketForSendDeque() {
  return _packetForSendDeque;
}
/**
...
void ChatSystem::setNextChatIdClient(const std::size_t &nextChatId) {
  _nextChatIdClient = nextChatId;
}

void ChatSystem::setFreeChatServerId(const std::size_t &freeChatId) {
  _freeChatServerIdSet.insert(freeChatId);
}
 ...
 */
void ChatSystem::eraseUser([[maybe_unused]] const std::shared_ptr<User> &user) {
  // Reserved hook for the full user-removal flow.
}
 ...
 */
std::size_t ChatSystem::showUserList(const bool showActiveUser) { // print the list of users
  ...
  for (const auto &user : _users) {
    ...
    if (!showActiveUser && user == _activeUser)
      continue;
    std::cout << index << ".  Name - " << user->getUserName() << ", login - " << user->getLogin() << ";" << std::endl;
    ++index;
  }
  return returnIndex;
}
