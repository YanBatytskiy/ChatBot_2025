...
#define PORT 50000          // Port number to use
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<User>> _loginUserMap; ///< Direct lookup of users by login.
  std::map<std::size_t, std::shared_ptr<Chat>> _chatIdChatMap;          ///< Direct lookup of chats by id.
...
public:
  ...
  */
   ...
   */
  std::vector<std::shared_ptr<User>> findUserByTextPart(const std::string &textToFind) const; // user search
   ...
   */
  std::size_t showUserList(const bool showActiveUser); // print the list of users
  /**
