...
private:
  ...
  // int64_t is the timeStamp
  // multimap sorts by key (time) automatically
  // allows multiple messages with the same timestamp
  ...
  // Direct lookup from messageId to timestamp inside the multimap storage.
  std::unordered_map<std::size_t, int64_t> _messageIdToTimeStamp;

  // Key: login
  // Value: set of messageIds the user has deleted in that chat view
