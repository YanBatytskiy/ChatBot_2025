...
void ScreenUserData::setUserDataToLabels(const QModelIndex &idx) {
  ...
  // simple user data view
  if (!status) {
    if (bunTo != 0 && bunTo > getCurrentDateTimeInt()) {
      ui->bunnedToDateUserLabel->setText("Banned until: " + QString::fromStdString(
    ...
    if (isActive) {
      ...
      ui->blockedUserLabel->setText("User is blocked.");
      ui->unblockPushButton->setEnabled(true);
      ui->blockPushButton->setEnabled(false);

      ui->banPushButton->setEnabled(false);
      ui->reasonDisableabel->setText("Block reason: " + reasonDisable);
    }
  }
  // mode for adding contacts to a new chat
  else {
    ui->unBunPushButton->setEnabled(false);
    ui->banPushButton->setEnabled(false);
    ui->unblockPushButton->setEnabled(false);
...
void ScreenUserData::slot_on_block_push_button_clicked() {
  ...
  dialog.setWindowTitle("Enter block reason");
  dialog.setLabelText("Block reason (required):");
  dialog.setTextValue("");
  ...
  if (!result) {
    QMessageBox::warning(this, " Error.", "Block failed");
  } else {
    QMessageBox::information(this, " Message", "Block applied");

    // replaced in the contacts model
...
void ScreenUserData::slot_on_unblock_push_button_clicked() {
  ...
  QMessageBox::information(this, "Message", "User unblocked");
...
void ScreenUserData::slot_on_bun_push_button_clicked() {
  ...
  QDialog dialog(this);
  dialog.setWindowTitle(tr("Pick date"));

  QVBoxLayout *layout = new QVBoxLayout(&dialog);

  QLabel *label = new QLabel(tr("Pick a date:"), &dialog);
  ...
  QMessageBox::information(this, "Message", "Ban applied");
...
void ScreenUserData::slot_on_unbun_push_button_clicked() {
  ...
  const bool ok = (QMessageBox::question(
                     this, "Unblock",
                     "Do you want to remove the ban from the user?",
                     QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes);
  if (!ok)
    return;
  ...
  QMessageBox::information(this, "Message", "Ban removed");
