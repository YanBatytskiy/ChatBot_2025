...
QVariant ChatListModel::data(const QModelIndex &index, int chatRole) const {
  ...
  switch (chatRole) {
    case ParticipantsChatListRole:
      return it.ParticipantsChatList;
    case InfoTextRole:
      return it.InfoText;
    ...
    case IsMutedRole:
      return it.isMuted;
    case LastTimeRole:
      return static_cast<qlonglong>(it.LastTime);
    case ChatIdRole:
      return static_cast<qlonglong>(it.ChatId);
  }
  return {};
}
...
QString ChatListModel::buildInfoTextForRow(const QString &chatIdStr, const QString &unreadCountStr, const QString &lastTimeStr) {
  ...
  infoText += ", new = " + unreadCountStr;
  infoText += ", last: " + lastTimeStr;
