...
private:
  ...
  // multimap automatically sorts by key (time)
  // allows multiple messages with the same time
  ...
  // Reverse lookup from message id to timestamp for quick message access.
  std::unordered_map<std::size_t, int64_t> _messageIdToTimeStamp;

  // Key is the login
  // Value is the set of messageId the user deleted
