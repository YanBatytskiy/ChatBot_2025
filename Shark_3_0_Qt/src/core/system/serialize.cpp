...
std::vector<std::uint8_t> serializePacketList(std::vector<PacketDTO> &packetList) {
  ...
  // Optional wire-format tracing for PacketListDTO is kept commented below.
  ...
  for (std::size_t i = 0; i < packetList.size(); ++i) {
    ...
    // Optional per-packet tracing for manual serialization debugging.
...
std::vector<std::uint8_t> serializeToBinary(const PacketDTO &packetDTO) {
  ...
  // Optional PacketDTO trace for binary serialization debugging.
  ...
  try {
    switch (packetDTO.structDTOClassType) {
      ...
      case StructDTOClassType::responceDTO: {
        ...
        // Optional trace for buffer growth during response serialization.
