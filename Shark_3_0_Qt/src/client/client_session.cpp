...
bool ClientSession::CreateAndSendNewChatCl(std::shared_ptr<Chat> &chat_ptr,
  ...
  for (const auto &participant : participants) {
    // temporary structure for filling
    ParticipantsDTO participantsDTO;

    // Fill user data for registration in the system
  ...
  if (result) {
    ...
    // then add the chat to the system
...
ChatSystem &ClientSession::getInstanceCl() {
  return _instance;
}
