...
std::multimap<int, std::string> DatabaseInitSqlBuilder::BuildChatFirstSql() const {
  ...
  std::string t1 = R"(with
	...
	message1_insert as (
		...
		select chat_created.chat_id, user_e_record.user_id, 'Hello!', 1743508800000
	...
	message2_insert as (
		insert into public.messages (chat_id, sender_id, message_text, time_stamp)
		select chat_created.chat_id, user_a_record.user_id, 'Hi! how are things?', 1743509100000
	...
	message3_insert as (
		insert into public.messages (chat_id, sender_id, message_text, time_stamp)
		select chat_created.chat_id, user_e_record.user_id, 'Good, how about some coffee?', 1743509220000
		from chat_created cross join user_e_record
		returning id as message_id
	),
...
std::multimap<int, std::string> DatabaseInitSqlBuilder::BuildChatSecondSql() const {
  ...
  message1_insert AS (
    INSERT INTO public.messages (chat_id, sender_id, message_text, time_stamp)
    SELECT chat_created.chat_id, user_elena.user_id, 'Hiiii everyone!?', 1743512400000
  ...
  message2_insert AS (
    INSERT INTO public.messages (chat_id, sender_id, message_text, time_stamp)
    SELECT chat_created.chat_id, user_alex.user_id, 'Stay well yourself!?', 1743512520000
  ...
  message3_insert AS (
    INSERT INTO public.messages (chat_id, sender_id, message_text, time_stamp)
    SELECT chat_created.chat_id, user_sergei.user_id, 'Howdy everyone.', 1743513015000
  ...
  message4_insert AS (
    INSERT INTO public.messages (chat_id, sender_id, message_text, time_stamp)
    SELECT chat_created.chat_id, user_elena.user_id, 'Where are we going?', 1743513129000
    FROM chat_created CROSS JOIN user_elena
    RETURNING id AS message_id
  ),
  ...
  message5_insert AS (
    ...
    SELECT chat_created.chat_id, user_sergei.user_id,
           'Come with me, across thrice nine lands, to the thrice tenth kingdom, to seek wisdom and happiness, to test our strength and fate, and as for what lies ahead, only the road will show',
           1743514380000
    FROM chat_created CROSS JOIN user_sergei
    RETURNING id AS message_id
  ),
