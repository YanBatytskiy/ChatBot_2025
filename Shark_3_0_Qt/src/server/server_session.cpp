...
void ServerSession::ProcessIncoming(SessionTransport &transport) {
  try {
    ...
    std::cerr << "Server: " << ex.what() << " = " << MESSAGE_LENGTH
              << std::endl;
...
bool ServerSession::sendPacketListDTO(SessionTransport &transport,
  ...
  try {
    ...
    std::cerr << "Server: Unknown error. sendPacketListDTO"
              << ex.what() << std::endl;
    return false;
  }
  return true;
  ...
  try {
    ...
    if (deletedMessagesMultiset.has_value() && deletedMessagesMultiset.value().size() > 0) {
      ...
      for (auto &participant : participants.value()) {
        ...
        // iterate over all user messages and append to the vector for sending
        if (std::distance(range.first, range.second)) {
          for (auto it = range.first; it != range.second; ++it) {

            // populate deletedMessageIds
...
std::optional<std::vector<ChatDTO>> ServerSession::FillForSendAllChatDTOFromSrvSQL(const std::string &login) {
  ...
  for (const auto &chat : chatList) {
    ...
    if (tempDTO.has_value())
      ...
      std::cerr << "Server: FillForSendAllChatDTOFromSrvSQL. Chat_id " << chat << " is not populated" << std::endl;
      continue;
    }
  } // first for
...
//
//
// Get the messages of a user for a specific chat
...
//
//
// Build and return in response to a user request: chats and messages
